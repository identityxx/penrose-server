...
PENROSE_MONITOR_PID="$PENROSE_HOME/logs/penrose-monitor.pid"
PID=0
RUNNING=0

if [ -f "$PENROSE_MONITOR_PID" ] ; then
  PID=`cat "$PENROSE_MONITOR_PID"`
  LINES=`ps -p $PID | wc -l`
  LINES=`expr $LINES`
  if [ "$LINES" = "2" ]; then
    RUNNING=1
  else
    rm -f "$PENROSE_MONITOR_PID"
  fi
fi
...
if [ "$1" = "start" ] ; then
  ...
  if [ "$RUNNING" = "1" ] ; then
    echo Penrose Monitor is running
    exit 1
  else
    shift
  ...
  if [ "$RUNNING" = "1" ] ; then
    ...
    echo Penrose Monitor is not running
    exit 1
  fi
