 ...
 */
public class NISAutomountEntry extends DynamicEntry {
    ...
    public void search(
        ...
        if (debug) {
            ...
            log.debug(TextUtil.displayLine("AUTOMOUNT SEARCH", 80));
    ...
    public void generateSearchResults(
        ...
        SearchResponse newResponse = new Pipeline(response) {
            public void add(SearchResult primaryResult) throws Exception {
                ...
                newResult.setEntry(NISAutomountEntry.this);
        ...
        if (getDn().matches(baseDn)) {
            ...
            RDN parentRdn = dn.get(1);

            String automountKey = (String)rdn.get("cn");
            String automountMapName = (String)parentRdn.get("nisMapName");

            primarySource.searchAutomountEntries(
                    session,
                    automountKey,
                    automountMapName,
                    newResponse
    ...
    public DN computeDn(
        ...
        AttributeMapping cn = getAttributeMapping("cn");
        rb.set("cn", interpreter.eval(cn));
        db.append(rb.toRdn());
