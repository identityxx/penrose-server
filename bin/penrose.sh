...
start() {
  if [ -f "$PENROSE_PID" ] ; then
    ...
    if [ -n "$CYGHOME" ] ; then
      exec "$JAVACMD" $PENROSE_DEBUG_OPTS $PENROSE_OPTS -classpath "$LOCALCLASSPATH" -Dpenrose.home="$PENROSE_HOME" -Dcygwin.user.home="$CYGHOME" org.apache.ldap.server.ServerMain "conf\\apacheds.xml" $PENROSE_ARGS "$@" >> "$PENROSE_HOME/var/penrose.out" 2>&1 &
    else
