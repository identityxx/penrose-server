...
start() {
  if [ -f "$PENROSE_PID" ] ; then
    ...
    if [ -n "$CYGHOME" ] ; then
      exec "$JAVACMD" $PENROSE_DEBUG_OPTS $PENROSE_OPTS -classpath "$LOCALCLASSPATH" -Djavax.management.builder.initial=mx4j.server.MX4JMBeanServerBuilder -Dpenrose.home="$PENROSE_HOME" -Dcygwin.user.home="$CYGHOME" org.safehaus.penrose.PenroseServer $PENROSE_ARGS "$@" >> "$PENROSE_HOME/var/penrose.out" 2>&1 &
    else
